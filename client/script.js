...
// CODE HERE
function helloBtnColorTwo (){
    sayHelloButton.style.backgroundColor = "#EFEFEF"
    sayHelloButton.style.color = "black"
}

sayHelloButton.addEventListener("mouseout", helloBtnColorTwo)
...
const ohMy = () => {
    // YOUR CODE HERE
    //sending a request to the server to send us http://localhost:3000/animals
    axios.get(`${baseURL}/animals`)
    // if we can retrieve that, then do this
    .then(function(res){
...
/*
    ...
    Write a function that makes a get request to 'http://localhost:3000/query-test', with a 
    query of your choice on the end!
...
function attachQuery() {
    axios.get(`${baseURL}/query-test/?name=sean&yama`)
    .then(res => {
        console.log(res.data)
    })
    .catch(err => console.log(err))
...
/*
    In the function that you wrote for Problem 8, change the URL to test a couple 
    different scenarios. 
    1: Send no queries on the URL -- what happened? It consoles "You sent an empty query!"
...
/*
    ...
    small form and write a function that makes a post request. Then you'll attach 
    that function to the submit event on the form. We'll be creating a list of foods. 
    ...
    the user to enter a food. And the button should indicate that it will add food into a list. 
    ...
    Now make an axios post request to /food. Inside the parentheses where you passed 
    the URL in, pass in body as the second argument. 
